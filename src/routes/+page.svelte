<script lang="ts">
	...
	const handleConsiderDuel = (evt: CustomEvent<DndEvent<Dueller>>) => {
		$duels = evt.detail.items;
	};

	const handleFinalizeDuel = (evt: CustomEvent<DndEvent<Dueller>>) => {
		$duels = evt.detail.items;
	};
	...
	function RemoveFromDuel(id: number) {
	...
	<div class="h-screen overflow-y-auto bg-muted/35">
		<div class=" flex border-b border-secondary">
			...
			<div class="mx-auto gap-2 p-4">
				<Button
					href="https://id.twitch.tv/oauth2/authorize?response_type=token&client_id=wryq0ptbcqok7crz9ajvlj25e05tqp&redirect_uri=http://localhost:4173/oauth&scope=user%3Aread%3Achat+channel%3Aread%3Aredemptions"
				>
			...
			<!-- <div> -->
			<!-- 	<Button on:click={BackupData}>Backup</Button> -->
			<!-- </div> -->
		</div>
		<!-- {userID} -->
